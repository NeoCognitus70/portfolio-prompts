...
| [`check-library.py`](check-library.py) | The library's **self-gate**: registry/classification safety, generated README, links, working norms, invocation paths, worklist format, deterministic tool-test scenarios, and handover pair integrity (when the sibling `session-notes/` archive is present). | PP-15, PP-17, PP-18, PP-25, PP-26 |
...
(`tools/tests/test_*.py`, covering workspace-preflight scenarios and the manifest builder's
pair-integrity gate) internally, so a stale table or a tooling regression fails the gate.
