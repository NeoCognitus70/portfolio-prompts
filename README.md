...
| `orangehrm-pim-automation` | GBrooks1970/orangehrm-pim-automation | Active | OrangeHRM PIM add-employee E2E (Serenity/JS + Playwright + Cucumber). **Backlog:** `docs/backlog.md` (simple table format, not the scored template). Gates: `npm test` — **requires the app up first via `docker compose up -d --wait`** (`docker-compose.yml`; seeded DB + `Conf.php`); CI (`e2e` workflow, Node 20) also builds Serenity living docs and deploys Pages. One open, deliberately non-blocking item (#4 — smoke-profile target for the read-only search scenario). Live docs: gbrooks1970.github.io/orangehrm-pim-automation. |
