...
| `parabank-bank-automation` | GBrooks1970/parabank-bank-automation | Active | Ninth portfolio project. ParaBank (Parasoft, Apache-2.0) as a pinned, Docker-backed banking SUT (never committed here; fetched at build). Two lanes: Serenity/JS + Playwright + Cucumber UI journeys (A1-A5) and API-first stateful BDD with a portfolio-first REST-SOAP parity check (B1-B4); 18-scenario baseline. Phased delivery PB-P0..P5 is complete; the project is active for PB-CODEX-01..10 review remediation in docs/backlog.md v7, with PBR-01/PBR-02 retained as monitoring risks. Decisions are in docs/decision-register.md (DR-PB-01..07); latest handover is v2 in session-notes/. Gates: per `ci.yml` — static `npm run typecheck`; E2E `pwsh ./scripts/build-sut.ps1; docker compose up -d; pwsh ./scripts/gate.ps1` then `npm run verify`. docs/project-contract.md is the first-hit gate source. npm run verify runs both lanes + smoke-safety + the Serenity report content check (needs Playwright Chromium and a JDK, both installed in CI); the boot gate precedes verify.. |
