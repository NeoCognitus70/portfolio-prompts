...
| `hand-baked-screenplay-pattern` | NeoCognitus70/hand-baked-screenplay-pattern | Resting | Showcase | Provider-switching Phases 0-1 completed 2026-08-17 as HBSP-28..33. The dependency-free v0.3.0 release supplies the compatibility baseline, portable Question/ability seams, extensible event envelope, conformance kit, and immutable ESM/CommonJS distribution. Zero provider items remain; Calculator Phase 2 completed on 2026-08-19, while later portfolio migrations remain unpromoted. Gates: `npm run verify`. |
| `calculator-screenplay-bdd` | NeoCognitus70/calculator-screenplay-bdd | Closed 2026-08-19 | Showcase | Closed 2026-08-19 (terminal handover v5 FINAL) with zero outstanding backlog items. Provider-switching Phase 2 CAL-22..25 is complete: the consumer pins immutable v0.3.0, owns one static composition gateway and alternate provider, and permanently gates a bounded nine-check dual-provider REST/conformance profile before the 42-test remainder. The project no longer requires a sibling checkout. Runtime hot-switching, report parity, and later provider-switching phases remain unpromoted. Static API docs: https://neocognitus70.github.io/calculator-screenplay-bdd/. Gates: `npm run verify`. |
| `gb.automation.smoketests.sudoku.poc` | GBrooks1970/gb.automation.smoketests.sudoku.poc | Resting; product cycle complete 2026-08-24 | Showcase | Multi-stack POC with its own doc system. All 93 backlog items are resolved: the three-Stack deterministic solver includes Naked Pairs and X-Wing, while DEMOAPP001 also supplies the interactive tutor and puzzle generator. Browser-only visualisation evidence is published at https://gbrooks1970.github.io/gb.automation.smoketests.sudoku.poc/. Run stack defaults inside the touched stack's directory, never at the repo root. Gates: per `ci.yml` — stack jobs (demoapp001 cypress/typescript; demoapp002 pytest/python; demoapp003 specflow/csharp) + `.batch/*.ps1` parity; run the job(s) for the stack(s) touched. Deviations: backlog `DOCS/.planning/backlog.md`, implementation-logs `DOCS/.implementation-logs/`, reviews `DOCS/.review/`, templates `DOCS/.templates/`. |
| `bfx-ws-screenplay` | GBrooks1970/bfx-ws-screenplay | Active | Showcase | In-repo `SPECIFICATION.md` is normative (SDD) — one SPEC unit at a time; feature file reviewed before framework code; deviations need an ADR change note. `test:smoke` hits the live public API; `environment-blocked` outcomes are platform maintenance, not failures; `@extended` runs nightly only. The implementation Kanban board, generated from `docs/backlog.md` and drift-gated in CI by a pinned `portfolio-kanban-generator`, is published at https://gbrooks1970.github.io/bfx-ws-screenplay/. Gates: `npm run typecheck`, `npm run lint`, `npm run test:smoke`. |
...
| `parabank-bank-automation` | GBrooks1970/parabank-bank-automation | Active | Showcase | Ninth portfolio project. ParaBank (Parasoft, Apache-2.0) as a pinned, Docker-backed banking SUT (never committed here; fetched at build). Two lanes: Serenity/JS + Playwright + Cucumber UI journeys (A1-A5) and API-first stateful BDD with a portfolio-first REST-SOAP parity check (B1-B4); the current baseline is 14 API and 8 UI scenarios plus 36 SUT-independent unit tests. Phased delivery PB-P0..P5, the PB-CODEX-01..10 review remediation and the PB-PIN-01..04 maintenance cycle are all complete (docs/backlog.md v28). The project stays active because three trigger-gated LOW risks remain open: PBR-01, PBR-04 and PBR-05, each waiting on a future upstream SUT pin bump (DR-PB-02). PBR-02, PBR-03, PBR-06 and PBR-07 are resolved. Two lane-behaviour facts a fan-out must not re-derive: under DR-PB-11 container-image registry drift only WARNS during a build and is fatal only in `build-sut.ps1 -ValidateImagePinsOnly` and the scheduled `pin-drift` workflow, which raises a tracking issue; and stale or unprovenanced performance evidence is withheld from the published /perf/ page rather than failing the deploy. Decisions are in docs/decision-register.md (DR-PB-01..11); latest handover is v5 in session-notes/. Gates: per `ci.yml` — static `npm run typecheck`; E2E `pwsh ./scripts/build-sut.ps1; docker compose up -d; pwsh ./scripts/gate.ps1` then `npm run verify`. docs/project-contract.md is the first-hit gate source. npm run verify runs both lanes + smoke-safety + the Serenity report content check (needs Playwright Chromium and a JDK, both installed in CI); the boot gate precedes verify.. |
| `juice-shop-dast-automation` | GBrooks1970/juice-shop-dast-automation | Closed 2026-09-10 | Showcase | Eleventh portfolio project, and the portfolio's first security lane. A delivered DAST showcase: an OWASP ZAP passive baseline scan plus BDD exploit-confirmation scenarios against a pinned, intentionally-vulnerable OWASP Juice Shop 20.1.1 (MIT) training target -- not a real product; its findings are expected and every surface is labelled. The positive-detection gate asserts the expected security-misconfiguration and information-disclosure classes (a passive baseline detects no injection classes), while four BDD scenarios (SQL-injection login bypass, confidential-file exposure, poison-null-byte filter bypass, and IDOR) supply the active-exploitation proof a passive scan cannot. All phases DAST-P0..P5, the Gemini review remediation, and DAST-M3 are complete. DAST-M3 added bounded browser-backed SPA discovery and was accepted by three independent fresh-container scans with identical seven-class gating sets (run 34200784522), with no rerun or weakening of the fail-closed contract. DAST-M4 (zero-vulnerability dependency remediation) is complete, bringing open backlog items to zero and transitioning the lifecycle to resting. R1/R2 are standing mitigations and DAST-M1/M2 are dormant conditional triggers, not open work. Design and decisions D2.1a-D2.7a are in docs/dast-lane-design.md; images are digest-pinned (Juice Shop 20.1.1, OWASP ZAP 2.17.0), and a pin bump re-triggers the Phase 0 probe (backlog DAST-M1). The labelled report is published at https://gbrooks1970.github.io/juice-shop-dast-automation/. Latest handover is v4 in session-notes/. Gates: per `ci.yml` — static `npm run verify`. npm run verify (typecheck + 21 unit tests) is Docker-free and is the orchestration-safe gate. The ZAP scan (npm run dast) and the BDD exploit confirmations (npm run bdd) are self-contained -- each boots the pinned Juice Shop container and tears it down -- and run in CI (.github/workflows/ci.yml), not in verify. The pass/fail signal is npm run scan:verdict (which parses report.json), not zap-baseline.py's exit code, which is always non-zero on this deliberately-vulnerable target.. |
...
| `saleor-graphql-automation` | GBrooks1970/saleor-graphql-automation | Active | Showcase | Fourteenth portfolio project (P-12). Production-grade GraphQL e-commerce automation showcase built on Saleor 3.23 (BSD-3-Clause) using the Screenplay Pattern, Serenity/JS, Cucumber, and TypeScript. Demonstrates contract testing via GraphQL Inspector schema diff gates, stateful BDD journeys across catalogue browsing, customer authentication, multi-line checkout, and staff fulfilment, with read-only smoke safety profiles, Docker Compose SUT orchestration, and comprehensive living documentation. Gates: `npm run verify`. |
