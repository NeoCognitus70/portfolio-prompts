...
| `bfx-ws-screenplay` | GBrooks1970/bfx-ws-screenplay | Active | WebSocket testing against the Bitfinex public WebSocket API v2 (Cypress + cucumber preprocessor + in-house Screenplay, chainable-native). **SDD project:** in-repo `SPECIFICATION.md` is normative — one SPEC unit at a time, feature file reviewed before framework code, deviations need an ADR change note. Gates: per `docs/project-contract.md` (`npm run typecheck`, `npm run lint`, `npm run test:smoke`). **`test:smoke` hits the live public API**: `environment-blocked` outcomes (`EnvironmentBlockedError`) are platform maintenance, not failures; `@extended` runs nightly only (live-API etiquette). |
| `markdown-renderer` | GBrooks1970/markdown-renderer | **Active (backlog pending)** | Static offline markdown viewer (5th project); live demo `gbrooks1970.github.io/markdown-renderer/`. Gates: `npm run verify` (per `ci.yml` = `tsc` + Vitest + Playwright). **No `docs/backlog.md` on `main` yet** (in-flight on branch `docs/add-backlog`) — until it merges, the source-of-truth-dependent prompts (`resume-session`, `derive-worklist`, `review-all-projects`, `close-project`) must stop with "not onboarded"; `write-handover` / `write-implementation-log` work now. Not coupled to other projects. |
| `orangehrm-pim-automation` | GBrooks1970/orangehrm-pim-automation | Active | OrangeHRM PIM add-employee E2E (Serenity/JS + Playwright + Cucumber; 6th project); live Pages `gbrooks1970.github.io/orangehrm-pim-automation`. Backlog `docs/backlog.md`. No root `verify` script — gates **per `ci.yml`**: static `npx tsc --noEmit` (`npm run typecheck`); full E2E brings the app up via `docker compose up -d --wait` then `npm test` (cucumber `default`, `not @deferred`). Not coupled to other projects. |
