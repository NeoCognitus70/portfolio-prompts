...
| `hand-baked-screenplay-pattern` | NeoCognitus70/hand-baked-screenplay-pattern | Resting | Showcase | Provider-switching Phases 0-1 completed 2026-08-17 as HBSP-28..33. The dependency-free v0.3.0 release supplies the compatibility baseline, portable Question/ability seams, extensible event envelope, conformance kit, and immutable ESM/CommonJS distribution. Zero provider items remain; Calculator Phase 2 was promoted in its owning backlog on 2026-08-18, while later portfolio migrations remain unpromoted. Gates: `npm run verify`. |
| `calculator-screenplay-bdd` | NeoCognitus70/calculator-screenplay-bdd | Active | Showcase | Active provider-switching Phase 2 pilot, promoted 2026-08-18 as four dependency-ordered MEDIUM items CAL-22..25: pin the v0.3.0 provider artefact, introduce one Calculator composition gateway, prove a bounded alternate-provider REST profile, then gate and document the evidence. The full suite remains hand-baked by default; runtime hot-switching and later portfolio migrations are out of scope. Gates: `npm run verify`. Depends on `hand-baked-screenplay-pattern` — prepare:screenplay installs and builds inside the sibling checkout (provider-first). |
